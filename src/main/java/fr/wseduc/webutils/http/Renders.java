...
public class Renders {
	...
	public static void renderError(HttpServerRequest request, JsonObject error) {
		renderError(request, error, HttpResponseStatus.INTERNAL_SERVER_ERROR.code(), "Internal Server Error");
	}

	public static void renderError(HttpServerRequest request, JsonObject error, final int code, final String statusMessage) {
		request.response().putHeader("content-type", "application/json");
		request.response().putHeader("Cache-Control", "no-cache, must-revalidate");
		request.response().putHeader("Expires", "-1");
